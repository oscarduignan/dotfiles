...
class ActionDelete {
    ...
    static byLines(args) {
        ...
        const document = activeTextEditor.document;
        let ranges = activeTextEditor.selections.map(selection => Range_1.UtilRange.toLinewise(selection, document));
        ranges = Range_1.UtilRange.unionOverlaps(ranges);
        return (args.shouldYank ? Register_1.ActionRegister.yankLines() : Promise.resolve(true))
            .then(() => {
            return activeTextEditor.edit((editBuilder) => {
                ranges.forEach((range) => editBuilder.delete(range));
            });
